...
namespace SupersonicWisdomSDK
{
    ...
    internal class SwAbConfig
    {
        ...
        public int durationDays = Constants.DEFAULT_DURATION_DAYS;
        ...
        {
            var today = SwUtils.TryParseDate(TodayString, SwConstants.SORTABLE_DATE_STRING_FORMAT);

            if (!today.HasValue) return -1;

            var startDayString = SwInfra.KeyValueStore.GetString(StartDateStorageKey(id), null);
            var startDay = SwUtils.TryParseDate(startDayString, SwConstants.SORTABLE_DATE_STRING_FORMAT);
            var passedDays = -1;

            if (startDay.HasValue)
            {
                passedDays = today.Value.Subtract(startDay.Value).Days + 1;
                SwInfra.Logger.Log($"{nameof(SwAbConfig)} | {nameof(CalculatePassedDays)} | passed days of A/B {id}: {passedDays}");
            }
        ...
        {
            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(RemainingDays)} | ab: {this}");
            var remainingDays = CalculateRemainingDays();
            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(RemainingDays)} | remainingDays: {remainingDays?.ToString() ?? "eternal"}");
            ...
            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(RemainingDays)} | ab key = {key} | ab value = {value}");
        ...
        private int? CalculateRemainingDays()
        {
            ...
            if (IsValid)
            {
                ...
                var terminationDate = SwUtils.TryParseDate(expirationDate, SwConstants.SORTABLE_DATE_STRING_FORMAT);
