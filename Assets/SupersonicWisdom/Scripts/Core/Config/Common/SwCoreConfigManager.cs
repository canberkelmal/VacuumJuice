...
namespace SupersonicWisdomSDK
{
    internal abstract class SwCoreConfigManager : ISwConfigManager, ISwNativeRequestListener, ISwLocalConfigProvider, ISwReadyEventListener, ISwGameStateSystemListener
    {
        ...
        public void HandleGameSystemStateChange(object sender, SwSystemStateEventArgs eventArgs)
        {
        ...
        {
            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(SetNewConfig)} | will set config: {configToSet}, ab: {ab}");
            Config = configToSet;
            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(SetNewConfig)} | did set config: {configToSet}, ab: {ab}");
        ...
        private void UpdateAb()
        {
            ...
            if (Config.ab.IsValid && !Config.ab.IsExpired)
            {
                ...
                SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(UpdateAb)} | abKey = {Ab.key} | abKey = {Ab.value}");
            }

            SwInfra.Logger.Log($"{nameof(SwCoreConfigManager)} | {nameof(UpdateAb)} | Config.ab: {Config.ab}");
        }
