...
namespace SupersonicWisdomSDK
{
    ...
    {
        #region --- Events ---
        private delegate void SwProgressionCallback(ESwLevelType swLevelType, long level, string customString, long attempts, long revives);
        ...
        private event Action<string> OnMetaStartedEvent;
        private event Action<string> OnMetaEndedEvent;
        ...
        internal SwGameStateSystem(SwCoreTracker tracker, SwCoreUserData coreUserData)
        {
            _tracker = tracker;
        ...
        public void ProcessTimeBasedGameStarted() 
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessTimeBasedGameStarted)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessLevelCompleted(ESwLevelType levelType, long level, string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessLevelCompleted)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessLevelFailed(ESwLevelType levelType, long level, string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessLevelFailed)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessLevelRevived(ESwLevelType levelType, long level, string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessLevelRevived)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessLevelSkipped(ESwLevelType levelType, long level, string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessLevelSkipped)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessLevelStarted(ESwLevelType levelType, long level, string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessLevelStarted)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessMetaStarted(string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessMetaStarted)} error: {e.Message}");
                }
            }
            else
            {
        ...
        public void ProcessMetaEnded(string customString)
        {
            ...
            if (transitionRequest)
            {
                ...
                {
                    SwInfra.Logger.LogError($"{nameof(SwGameStateSystem)} : {nameof(ProcessMetaEnded)} error: {e.Message}");
                }
            }
            else
            {
        ...
        {
            SwInfra.Logger.Log($"{nameof(SwGameStateSystem)} | {nameof(OnGameStateTransitionRequested)} | Attempting Transition from {_currentGameState} to {eventArgs.NewGameState} | reason: {reason} | end status level: {eventArgs.StateEndStatusRequest}");

            if (!ShouldTransitionOccur(eventArgs))
            {
                SwInfra.Logger.Log($"{nameof(SwGameStateSystem)} | {nameof(OnGameStateTransitionRequested)} | Transition blocked! | {eventArgs}");
                
                return false;
            }

            SwInfra.Logger.Log($"{nameof(SwGameStateSystem)} | {nameof(OnGameStateTransitionRequested)}: Successful transition - Current Game State: {_currentGameState}");
        ...
        private void NotifyGameStateListeners()
        {
            ...
            {
                listener.HandleGameSystemStateChange(this, updatedEventArgs);
            }
        }
        ...
        private SwSystemState.EGameState ConvertLevelTypeToGameState(ESwLevelType levelType)
        {
            switch (levelType)
            {
        ...
        private void OfficiateTransition(SwSystemState.EGameState? gameState, SwSystemState.EStateEvent currentStateEvent)
        {
            ...
            SwInfra.Logger.Log($"{nameof(SwGameStateSystem)} | {nameof(OfficiateTransition)} | {nameof(_currentGameState)}: {_currentGameState} | {nameof(_currentStateEvent)}: {_currentStateEvent} | {nameof(PreviousGameState)}: {PreviousGameState}");
        }
        ...
        {
            SwInfra.Logger.Log($"{nameof(SwGameStateSystem)} | {nameof(HandleUnauthorizedGameTransition)} | {nameof(swProgressEvent)}: {swProgressEvent} | {nameof(customString)}: {customString} | {nameof(levelType)}: {levelType} | {nameof(level)}: {level} "
